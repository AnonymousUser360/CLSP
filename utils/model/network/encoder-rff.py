...
class GaussianEncoder(nn.Module):
    def __init__(self, embedding_dim, sigma=1):
        ...
        if self.Hierachical:
            ...
            self.gaussian_array_2 = torch.nn.Parameter(self.get_gaussian_array(sigma=0.25), requires_grad=False)
    ...
    def forward(self, x: torch.Tensor):
        if len(x.shape) == 1:
            ...
            if self.Hierachical:
                ...
                # v3 = torch.zeros(size=(x.shape[0], self.embedding_dim*2), dtype=torch.float, device=x.device)                        
                # vp_3 = 2*np.pi*x*self.gaussian_array_3.T
                # v3[:, 0::2] = torch.sin(vp_3)  # even
                # v3[:, 1::2] = torch.cos(vp_3)  # odd
                # v4 = torch.zeros(size=(x.shape[0], self.embedding_dim*2), dtype=torch.float, device=x.device)                        
                # vp_4 = 2*np.pi*x*self.gaussian_array_4.T
                # v4[:, 0::2] = torch.sin(vp_4)  # even
                # v4[:, 1::2] = torch.cos(vp_4)  # odd                   
                # v = (v+v1+v2+v3+v4)/5.             
        elif len(x.shape) == 2:
            v = torch.zeros(size=(x.shape[0],x.shape[1],  self.embedding_dim*2), dtype=torch.float, device=x.device)            
            x = x.unsqueeze(dim=-1)
            ...
            if self.Hierachical:
                ...
                # v3 =torch.zeros(size=(x.shape[0],x.shape[1],  self.embedding_dim*2), dtype=torch.float, device=x.device)                                  
                # # print(f"x shape:{x.shape}, self.gaussian_array:{self.gaussian_array.shape}, self.gaussian_array_1:{self.gaussian_array_1.shape},v:{v.shape},v1:{v1.shape}")
                # vp_3 = 2*np.pi*x*self.gaussian_array_3
                # v3[:, :,0::2] = torch.sin(vp_3)  # even
                # v3[:, :,1::2] = torch.cos(vp_3)  # odd
                # v4 =torch.zeros(size=(x.shape[0],x.shape[1],  self.embedding_dim*2), dtype=torch.float, device=x.device)                                                     
                # vp_4 = 2*np.pi*x*self.gaussian_array_4 
                # v4[:, :,0::2] = torch.sin(vp_4)  # even
                # v4[:, :,1::2] = torch.cos(vp_4)  # odd                   
                # v = (v+v1+v2+v3+v4)/5.   
        return v
