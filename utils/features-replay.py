...
class Features:
    ...
    def transform_backpack_info(self,info,item_id2item_category,bullet_cnt_dict,scalar_info):
        ...
        backpack_item_info = {             
            ...
            'count': all_backpack_item[:,3].long(),# 可使用二进制进行编码为 < 256的数 bianry 8
            'size': (all_backpack_item[:,4]/self.backup_volume_norm).float(), # 物品占用背包容量 可除200   
    ...
    def transform_supply_info(self,info, own_position, safety_area, state,own_player_state, scalar_info):
        ...
        supply_item_info = {
            ...
            'size':(all_supply_item[:, 13].float()*all_supply_item[:, 1])/self.backup_volume_norm, ##TODO
    ...
    def transform_history_position_heatmap(self,own_player_state,top2bottom_teammate_position,
        ...
        # a = time.time()
        heatmap_info = self.heatmap_tool.generate_heat_map(position_info,server_is_0913 = position_info["server_is_0913"])
    ...
    def transform_event_info(self,state, own_player_state,own_position,state_info):
        ...
        event_info = {
            ...
            'time_delta': ((history_event_info[:, 6] ) / (1000*self.MAX_GAME_TIME)).float(),    # 
    ...
    def transform_obs(self, state, objs, depth_map, last_state, last_action, info,search_target_player_id = None):
        ...
        # 添加多久没看见人的信息
        scalar_info["not_visble_enemy_time"] = torch.tensor((state.timestamp - self.not_visble_enemy_time) / (1000*self.MAX_GAME_TIME), dtype=torch.float)
